...
- **프로젝트명**: 오미조이 (OMIJOY)
- **팀명**: OMIJA
- **개발 기간**: 2025년 10월 29일 ~ 2025년 12월 10일
...
---
✨ 주요 기능
공연 탐색부터 위치 기반 공연장 정보, 즐겨찾기/플래그 관리, 관리자 대시보드까지 통합 제공됩니다.
...
🚀 CI/CD & 배포 자동화
- **GitHub Actions**를 이용해 프론트/백엔드 각각 CI/CD 파이프라인 구성
- `release` 브랜치에 코드가 푸시되면 자동으로:
  - 애플리케이션 빌드 (Frontend: Vite, Backend: Gradle)
  - Docker 이미지 빌드 후 **Docker Hub**에 푸시
- **GCP Compute Engine** VM에서 최신 이미지를 pull 한 뒤
  - `docker-compose`를 통해 기존 컨테이너 중단 및 새 컨테이너 재실행
- 개발자는 코드 푸시만으로 **자동 빌드 · 배포 · 재기동**까지 한 번에 처리 가능
